    ...
    void SpaceshipController::Update() {
        if (this->fuel > 0.0f) {
            ...
            if (cmd.thrusting) {
                ...
                this->kinematic->set_velocity(this->kinematic->get_velocity() + forward * thrust_accel * dt);
            }
            ...
            if (cmd.rolling_left == false && cmd.rolling_right == false) {
                ...
                if (roll_velocity > 1e-6f) {
                    ...
                    float auto_roll_power = std::min(abs_correction, roll_accel);
            ...
            this->angular->euler_angles().y = spherical.delta_theta * this->horizontal_steer_accel;
            this->angular->euler_angles().x = spherical.delta_phi * this->vertical_steer_accel;
        } else {
